...
def generate_pdf_for_student_application(request, id):
    ...
    # print('APPLICATION ID FROM POST: ', id)
    # # get application details
    # application_details = Admission.objects.get(pk=id)

    # print('APPLICATION ID FROM DB: ', application_details.id)

    # # config = pdfkit.configuration(wkhtmltopdf='schoolapp/wkhtmltopdf/bin')

    # # Get the data to be used in the PDF
    # # data = {'name': 'John', 'age': 35, 'email': 'john@example.com'}
    # data = {'application_details': application_details}
    
    # # Render the HTML template with the data
    # template = get_template('schoolapp/reports/student_application_report.html')
    # print('TEMPLATE DATA: %s' % template.render(data))
    # html_string = template.render(data)
    ...
    # print('PDF DATA: %s' % pdf)
    
    # # Create an HTTP response with the PDF file
    # response = HttpResponse(pdf, content_type='application/pdf')
    # response['Content-Disposition'] = 'attachment; filename="student_application_report.pdf"'
    # print('HTTP RESPONSE DATA: %s' % response)
...
@login_required
def add_score_for(request, id, course_id):
    """
    ...
    if request.method == 'POST':
        ...
        return HttpResponseRedirect(reverse_lazy('add_score_for', kwargs={'id': id, 'course_id': course_id}))
    return HttpResponseRedirect(reverse_lazy('add_score_for', kwargs={'id': id, 'course_id': course_id}))
